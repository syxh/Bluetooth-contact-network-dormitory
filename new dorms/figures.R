...
paneller_CI = function(file,dormID,scenario_id)
{
  ...
  {
    output <- read.csv(paste0('output/data/Scenario_',scenario_id,'_0.59.csv'))
  }else{
    output <- read.csv(paste0('output/data/Scenario_',scenario_id,'_0.91.csv'))
  }
  t  = c(1:nrow(output))
...
paneller = function(file,dormID,scenario_id)
{
  ...
  # grid.text(label=paste0("A New Dorm"), x = unit(1,'npc')-unit(2.5,'lines'),
  #           y = unit(1,'lines'), gp=gpar(fontsize=12))
  # if(!logy)yaxis(ylm)
  # if(logy)yaxis(ylm,log = TRUE)

  # grid.yaxis(at=seq(0,1,0.2),label=seq(0,100,20))
  grid.text(ylabel,x=unit(0,'npc')+unit(ylabel_gap,'lines'),y=unit(0.5,'npc')-unit(0.5,'lines'),rot=90)
  grid.text('Time (day)', x= unit(0.5,'npc'), y=unit(0,'npc')+unit(-2,'lines'))
...
paneller(file='output/plot/acacia_new_dorms_ci.png',dormID = 1,scenario_id = 1)
paneller(file='output/plot/cassia_new_dorms_ci.png',dormID = 2,scenario_id = 1)
...
# 
# paneller(file='acacia_policies_s3_ci.png',dormID = 1,scenario_id = 3)
# paneller(file='cassia_policies_s3_ci.png',dormID = 2,scenario_id = 3)
# 
# paneller(file='acacia_policies_s4_ci.png',dormID = 1,scenario_id = 4)
# paneller(file='cassia_policies_s4_ci.png',dormID = 2,scenario_id = 4)
# 
# paneller(file='acacia_policies_s5_ci.png',dormID = 1,scenario_id = 5)
# paneller(file='cassia_policies_s5_ci.png',dormID = 2,scenario_id = 5)
# 
# paneller(file='acacia_policies_s6_ci.png',dormID = 1,scenario_id = 6)
# paneller(file='cassia_policies_s6_ci.png',dormID = 2,scenario_id = 6)
