...
fn <- list.files(paste0(getwd(),'/working'))
...
for(scenario_id in 1){
  ...
  {
    tmp1 <- read.csv(paste0('working/',split_fn_acacia[[scenario_id]][[i]]))
    tmp1 <- tmp1[,-c(1,122)]
    tmp1[1,1] <- 20
    tmp1 <- cumsum(as.numeric(tmp1))
    combine_acacia <- cbind(combine_acacia,tmp1)
    
    tmp2 <- read.csv(paste0('working/',split_fn_cassia[[scenario_id]][[i]]))
    tmp2 <- tmp2[,-c(1,122)]
    tmp2[1,1] <- 20
    tmp2 <- cumsum(as.numeric(tmp2))
    combine_cassia <- cbind(combine_cassia,tmp2)
  }
  write.csv(combine_acacia,file=paste0('working/combine_Scenario_',scenario_id,'_theta_acacia.csv'),row.names = F)
  CI <- gen_CI(combine_acacia)
  # CI$doy <- c(1:nrow(CI))
  write.csv(CI,paste0('output/data/Scenario_',scenario_id,'_acacia.csv'),row.names = FALSE)
  
  write.csv(combine_cassia,file=paste0('working/combine_Scenario_',scenario_id,'_theta_cassia.csv'),row.names = F)
  CI <- gen_CI(combine_cassia)
  # CI$doy <- c(1:nrow(CI))
  write.csv(CI,paste0('output/data/Scenario_',scenario_id,'_cassia.csv'),row.names = FALSE)
}
