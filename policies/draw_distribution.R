...
find_contact<- function(case_id, contact_matrix, sim_status,sim_params) 
{
  ...
  id <- lapply(case_id,function(i) {
    # as.numeric(substring(names(contact_matrix[i,-remove_id][which(contact_matrix[i,-remove_id]==1)]),2))
    allcontact <- which(contact_matrix[i,]==1)
...
draw_sec_infects_df <- function(state_df, sim_params, sim_status, import=FALSE){
  ...
  #   update_contact_ind_index <- contact_ind_index[-unique(c(sim_status$alrdy_infected,sim_status$isolation_contact))]#contact_ind_index[-state_df$case_id] # here, need to remove all infected case ids
  # }else{
  #   update_contact_ind_index <- contact_ind_index
  # }
  ...
  for(i in seq_along(state_df$case_id)){
    if(n_infect_reg[i]<1|all(trans_tree_regular[state_df$case_id[i],state_df$contact_id_regular[[i]]]<0.00000001)) {new_case_id_reg[[i]]=integer(0)
    }else{
      # new_case_id_reg[[i]] <- sample(update_contact_ind_index,#contact_ind_index[-case_id],
      ...
      new_case_id_reg[[i]] <- sample(state_df$contact_id_regular[[i]],#contact_ind_index[-case_id],
                                     n_infect_reg[i],replace=FALSE, prob = trans_tree_regular[state_df$case_id[i],state_df$contact_id_regular[[i]]])
      # if(length(new_case_id_reg[[i]])>0){
      #   update_contact_ind_index <- update_contact_ind_index[-new_case_id_reg[[i]]]
      # }
    }
  }
  ...
  set.seed(1111)
   ...
   for(i in seq_along(state_df$case_id)){
    if(n_infect_trans[i]<1|all(trans_tree_transient[state_df$case_id[i],state_df$contact_id_transient[[i]]]<0.00000001)) {new_case_id_trans[[i]]=integer(0)}else{
      new_case_id_trans[[i]] <- sample(state_df$contact_id_transient[[i]],#contact_ind_index[-case_id],
                                     n_infect_trans[i],replace=FALSE, prob = trans_tree_transient[state_df$case_id[i],state_df$contact_id_transient[[i]]])
